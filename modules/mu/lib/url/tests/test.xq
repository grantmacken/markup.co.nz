...
import module namespace url="http://markup.co.nz/#url" at "../url.xqm";

(:~

Its purpose is to test functions
when developing a xquery library
...
let $module := xs:anyURI('../url.xqm')

let $test-name := request:get-parameter('test', 'main')
let $test-url :=  xs:anyURI( $test-name || ".xqm" )


let $inspectModule :=  inspect:inspect-module( $module  )
...
:)
	...
	let  $rows := switch ($returnType)
		...
		case "node" return (
				  ...
				  return sum( ( 1 ,  $outNodesCount , $textNodesCount ) )
